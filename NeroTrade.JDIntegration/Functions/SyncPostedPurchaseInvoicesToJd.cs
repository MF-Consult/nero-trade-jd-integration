    ...
    ILogger<SyncPostedPurchaseInvoicesToJd> logger)
{
    ...
    //
    // 30 s, not 60: the heartbeat quantizes the configured cadence upwards, so with a one-minute heartbeat
    // the 72 s day cadence would land on 120 s instead of the ~90 s the open-order path runs at. Change the
    // cadence in config; this cron only bounds how finely that cadence can be expressed.
    ...
    public async Task RunAsync([TimerTrigger("*/30 * * * * *")] TimerInfo timer, CancellationToken cancellationToken)
    {
        ...
        catch (UnicontaConnectionUnavailableException ex)
        {
            ...
            logger.LogWarning(ex, "Uniconta unavailable; skipping this SyncPostedPurchaseInvoicesToJd tick");
        }
        catch (Exception ex)
        {
            run.MarkFailed(ex);
