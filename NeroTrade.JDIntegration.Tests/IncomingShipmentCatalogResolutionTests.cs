...
/// <summary>
/// Pins the purchase-order → incoming-shipment item-matching contract after the 2026-06 "Ukendt"
/// incident: JD matches a line to a catalog item ONLY via <c>catalog.id</c>, so a line whose SKU
/// is not in JD's catalog must fail loudly (clear failure carrying the SKU) instead of being sent
/// with a bogus id that JD silently registers as "Ukendt".
/// </summary>
public class IncomingShipmentCatalogResolutionTests
{
    [Fact]
    public async Task SkuInCatalog_ResolvesCatalogIdAndSendsShipment()
    {
        ...
        var shipment = MapPo(99, ("ITEM-1", "PC-001"));
    ...
    [Fact]
    public async Task SkuMissingFromCatalog_FailsShipmentAndSendsNothing()
    {
        // Catalog is empty → the line cannot be matched.
        var repo = new FakeJdRepository();
        var service = BuildService(repo);

        var shipment = MapPo(100, ("UNKNOWN-SKU", "PC-999"));

        var result = await service.CreateIncomingShipmentsAsync(new[] { shipment }, CancellationToken.None);

        Assert.Equal(0, result.SuccessCount);
        Assert.Empty(repo.SentShipments); // never POSTed a line JD would register as "Ukendt"
        var failure = Assert.Single(result.Failures);
        Assert.Contains("UNKNOWN-SKU", failure.Message);
    }

    [Fact]
    public async Task SkuWithSurroundingWhitespace_MatchesTrimmedCatalogKey()
    {
        // The catalog cache is keyed on the trimmed SKU (ItemMapper trims). A PO line whose SKU has
        // stray whitespace must still resolve, not silently fall through to a failed match.
        var repo = new FakeJdRepository
        {
            CatalogItems = { new JdCatalogItem { id = 777, sku = "ITEM-2" } }
        };
        var service = BuildService(repo);

        var shipment = MapPo(101, ("  ITEM-2  ", null));
    ...
    private static JdIncomingShipmentCreate MapPo(int purchaseNumber, params (string sku, string? externalSku)[] lines)
    {
        ...
        return new PurchaseOrderMapper().Map(po);
    }
